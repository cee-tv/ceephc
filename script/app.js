...
const mobileSplashScreen = document.getElementById('mobile-splash-screen');
...
function initApp() {
    ...
    if (shaka.Player.isBrowserSupported()) {
        ...
        const fullscreenButton = document.querySelector('.fullscreen-button'); // Use existing button
        if (fullscreenButton) {
...
// Function to start playback and initial setup
function startPlayback() {
    // Only initialize Shaka Player and load first channel once
    if (!shakaPlayer) {
        initApp();
    }
            ...
            if (tapTimeDiff < 500) {
                ...
                if (!document.fullscreenElement && tapCount === 1) {
                    ...
                    if (screen.requestFullscreen) {
                        ...
                        screen.webkitRequestFullscreen().then(() => {
                            body.classList.add('fullscreen-active');
                        }).catch(err => console.error("Error entering fullscreen:", err));
                    } else if (screen.msRequestFullscreen) {
                        screen.msRequestFullscreen().then(() => {
                            body.classList.add('fullscreen-active');
                        }).catch(err => console.error("Error entering fullscreen:", err));
                    }
                }
...
function isMobile() {
    // Check for screen width typically associated with mobile devices
...
document.addEventListener('DOMContentLoaded', () => {
    if (isMobile()) {
        // Hide TV container initially
        if (tvContainer) {
            tvContainer.style.display = 'none';
        }
        
        // Show splash screen
        if (mobileSplashScreen) {
            mobileSplashScreen.style.display = 'flex';
            startTvButton.addEventListener('click', () => {
                // Hide splash screen
                mobileSplashScreen.style.display = 'none';
                ...
                // Add fullscreen-active class immediately after requesting fullscreen
                body.classList.add('fullscreen-active');
                ...
                if (screen.orientation && screen.orientation.lock) {
                    screen.orientation.lock('landscape').catch((error) => {
                        ...
                        // This might happen if user explicitly denies, or browser doesn't support
                        // or if the page is not in fullscreen already (which it should be after requestFullscreen)
                    });
                } else {
                    console.warn('Screen orientation API not supported or lock not allowed.');
                }
        ...
        // For desktop, just start playback directly
        startPlayback();
    }

    // Listen for fullscreen changes to manage 'fullscreen-active' class on body
...
function toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
        ...
        // The fullscreenchange listener will remove the class.
    }
}
...
function toggleInterface(type) {
    ...
    switch(type) {
        case 'remote':
            ...
            break;
        case 'channels':
            toggleSidebar(); // Use the same sidebar toggle logic
            break;
        case 'keyboard':
            keyboardOverlay.style.display = keyboardOverlay.style.display === 'block' ? 'none' : 'block';
            break;
...
document.addEventListener('click', (event) => {
    // Check if the click is outside any open interface and not on their toggle buttons
    const isClickOutsideChannelList = channelList.style.display !== 'none' && 
                                      !channelList.contains(event.target) && 
                                      !toggleMenuBtn.contains(event.target) &&
                                      !sidebarToggle.contains(event.target);

    const isClickOutsideRemoteControl = !remoteControl.classList.contains('hidden') && 
                                        !remoteControl.contains(event.target) && 
                                        !toggleMenuBtn.contains(event.target);

    const isClickOutsideMenuOverlay = menuOverlay.style.display !== 'none' && 
                                      !menuOverlay.contains(event.target) && 
                                      !toggleMenuBtn.contains(event.target);

    const isClickOutsideKeyboardOverlay = keyboardOverlay.style.display !== 'none' && 
                                          !keyboardOverlay.contains(event.target) && 
                                          !toggleMenuBtn.contains(event.target);

    if (isClickOutsideChannelList) {
        channelList.classList.remove('visible');
        // Also remove the open class from sidebar-toggle to update arrow direction
    ...
    if (isClickOutsideRemoteControl) {
        remoteControl.classList.add('hidden');
    }

    if (isClickOutsideMenuOverlay) {
        menuOverlay.style.display = 'none';
    }

    if (isClickOutsideKeyboardOverlay) {
        keyboardOverlay.style.display = 'none';
    }
});

// Add click stop propagation for overlays to prevent clicks inside them from closing them
